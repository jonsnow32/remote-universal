 ...
 */
export function parseProntoHex(hex) {
    ...
    if (words[0] !== 0x0000) {
        throw new Error(`[IRModule] Unsupported Pronto mode 0x${words[0].toString(16).padStart(4, '0')} — only raw mode (0x0000) is supported`);
    }
    const freqDivisor = words[1];
    if (freqDivisor === 0)
        throw new Error('[IRModule] Pronto Hex frequency divisor is zero');
    // carrier_Hz = 1_000_000 / (divisor × 0.241246 µs) ≈ 4_145_152 / divisor
    const frequencyHz = Math.round(4145152 / freqDivisor);
    ...
    if (pairsToSend === 0)
        throw new Error('[IRModule] Pronto Hex contains no burst pairs');
    const tickDurationUs = 1000000 / frequencyHz;
    const pattern = [];
...
function getNative() {
    return NativeModules.IRBlaster ?? null;
}
function getUSBNative() {
    return NativeModules.USBIRBlaster ?? null;
}
// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------
...
export const IRModule = {
    ...
    async isAvailable() {
        ...
        try {
            ...
            if (native && (await native.hasIrEmitter()))
                return true;
            // 2. Fall back to USB IR blaster
            const usb = getUSBNative();
            if (usb && (await usb.isConnected()))
                return true;
            return false;
        }
        catch {
     ...
     */
    async transmit(_deviceId, payload) {
        if (Platform.OS !== 'android') {
        ...
        throw new Error('[IRModule] No IR transmitter available. ' +
...
//# sourceMappingURL=IRModule.js.map