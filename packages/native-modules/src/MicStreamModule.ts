...
const native = NativeModules.MicStream as MicStreamNative | null | undefined;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const emitter = native ? new NativeEventEmitter(native as any) : null;

// ─── Public API ───────────────────────────────────────────────────────────────

export type MicChunkListener = (base64Pcm: string) => void;
...
export const MicStreamModule = {
   ...
   */
  onChunk(listener: MicChunkListener): () => void {
    if (!emitter) return () => {};
